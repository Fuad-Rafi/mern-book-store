...
import { analyzeQuery, generateAssistantReply } from '../services/llmService.js';
...
const buildGroundedRecommendations = (books = []) => {
  return books.map((book) => ({
    ...
    exceedsBudget: Boolean(book.exceedsBudget),
  }));
};
...
router.post('/chat', authenticateToken, requireRole('customer'), assistantChatRateLimiter, async (req, res) => {
  try {
    ...
    // Intent is classified on the RAW message, not on a rewritten one: a
    // greeting rewritten into a standalone search query stops looking like a
    // greeting, and it short-circuits before any LLM call is paid for.
    ...
    if (intent.isGreeting || intent.isEmpty || intent.isClarification) {
      ...
      // One structured call does query rewriting AND constraint extraction.
      // Falls back to the regex extractor when the provider is unavailable.
      ...
      const retrieval = await retrieveRelevantBooks({
        userId: req.user.id,
        userQuery: analysis.standaloneQuery,
        limit,
      ...
      retrievedContext = {
        ...
        budgetExceeded: Boolean(retrieval.budgetExceeded),
      ...
      if (!retrieval.retrievedBooks.length) {
        ...
        refinementHints = llmResult.refinementHints || [];
        retrievedContext.replySource = llmResult.source;
        retrievedContext.grounded = llmResult.grounded !== false;
    ...
    return res.status(200).json({
      ...
      // Generated by the model on every call; it used to be parsed and thrown
      // away, so the tokens were paid for and never used.
      refinementHints,
      retrievedContext,
